...
export function MemberCard(props: { user: ClientUser; events: Event[] }) {
    const sortedEvents = props.events
        .filter((event) => event.date)
        .sort((a, b) => a.date!.getTime() - b.date!.getTime());

    return (
        <div className="border-border bg-bg-muted flex flex-col gap-4 rounded-lg border p-6">
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-2">
                        {sortedEvents.map((event) => (
                            <div
                                key={event.id}
                                className="border-border bg-bg rounded border p-3"
                            >
